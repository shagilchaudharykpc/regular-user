...
def get_user_projects(UserName):
    try:
        ...
        FROM Users u
        JOIN UserProjectRoles up ON u.UserID = up.UserID
...
@app.route('/userprojectsroles', methods=['GET'])
def get_user_projects_roles():
    try:
        # Create a cursor object to execute SQL queries
        cursor = cnxn.cursor()

        # Execute the SQL query to get all projects
        cursor.execute('SELECT * FROM UserProjectRoles')
...
def add_user():
    try:
        ...
        # Validate input
        if not all([user_name, role_name, project_name, company_name]):
        ...
        cursor.execute("SELECT UserID FROM Users WHERE UserName=? AND CompanyID=?", user_name, company_id)
        existing_user = cursor.fetchone()
        if existing_user:
            return jsonify({'message': 'User already exists.'}), 409

        # Add user to database
        cursor.execute("INSERT INTO Users (UserName, CompanyID) VALUES (?, ?, ?)", user_name, company_id)
        cnxn.commit()
        ...
        cursor.execute("INSERT INTO UserProjectLink (UserID, ProjectID, RoleID) VALUES (?, ?, ?)", user_id, project_id, role_id)
        cnxn.commit()
...
def update_user_name():
    try:
        ...
        old_user_name = request.json['old_user_name']
        ...
        # If 'new_user_name', 'old_user_name' keys are not found in request body
        return jsonify({'error': 'Missing required parameter: new_user_name or old_user_name'}), 400
